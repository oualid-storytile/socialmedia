...
use Storytile\Socialmedia\OAuthOne\Client\Signature\HmacSha1Signature;
use Storytile\Socialmedia\OAuthOne\Client\Signature\SignatureInterface;
//use Throwable;
...
abstract class Server
{
    ...
    {
        // Pass through an array or client credentials
     ...
     * @param bool             $force
     *
     * @return \Storytile\Socialmedia\OAuthOne\Client\Server\User
     */
    ...
    protected function createClientCredentials(array $clientCredentials)
    {
        ...
        $clientCredentialsObj = new ClientCredentials();

        $clientCredentialsObj->setIdentifier($clientCredentials['identifier']);
        $clientCredentialsObj->setSecret($clientCredentials['secret']);

        if (isset($clientCredentials['callback_uri'])) {
            $clientCredentialsObj->setCallbackUri($clientCredentials['callback_uri']);
        }

        return $clientCredentialsObj;
    }
    ...
    /**
     * Handle a bad response coming back when using wrong credentials.
     *
     * @param BadResponseException $e
     *
     * @return void
     ...
     * @throws CredentialsException
     */
    public function handleCredentialsBadResponse(BadResponseException $e)
    {
        $response = $e->getResponse();
        $body = $response->getBody();
        $statusCode = $response->getStatusCode();
        ...
        throw new CredentialsException(
            "Received HTTP status code [$statusCode] with message \"$body\" when using credentials."
        );
    }
